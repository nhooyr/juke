	...
	for i := min; i != end; i += inc {
		...
		for j := min; j != end; j += inc {
			if j != i {
				...
				if g.s[j].on(g.s[i].bs[0].p, 0, len(g.s[j].bs), 1) || (len(g.s[i].bs) == 1 && oppositeDir(g.s[i].bs[0].d, g.s[j].bs[0].d) && (g.s[i].on(g.s[j].oldBs[0].p, 0, len(g.s[i].bs), 1) || g.s[j].on(g.s[i].bs[0].p, 0, len(g.s[j].bs), 1))) {
					g.s[i].die()
				}
			}
		}
